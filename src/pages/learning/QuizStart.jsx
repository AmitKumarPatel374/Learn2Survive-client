...
const QuizStart = () => {
  ...
  const handleStartQuiz = async () => {
    ...
    try {
      ...
      const response = await apiInstance.post(`/quiz/${quizId}/start`)

      if (response.data.success) {
        navigate(`/learning/quiz/${response.data.attemptId}`)
      }
    } catch (error) {
      ...
      <div className="mx-auto max-w-5xl">
        ...
        <div className="mb-4 flex items-center justify-between">
          ...
          <div className="text-right">
            ...
            <p className="text-sm text-[#8e909f]">{quiz.totalQuestions} Questions</p>
          </div>
        </div>
        ...
        <div className="relative overflow-hidden rounded-3xl border border-white/10 bg-[#171f33]/60 p-8 text-center backdrop-blur-xl lg:px-14">
          ...
          {/* Title */}
          <h1 className="relative z-10 text-4xl font-bold text-white">{quiz.title}</h1>
          ...
          <div className="relative z-10 mt-8 grid gap-5 md:grid-cols-3">
            <div className="rounded-2xl border border-white/10 bg-[#131b2e] p-6">
              <HelpCircle
                ...
                size={32}
              />
              <h3 className="font-semibold">{quiz.totalQuestions} Questions</h3>
            </div>
