...
const colorMap = {
  ...
  "State Disaster Management Authority": "bg-indigo-500/10 text-indigo-400",
  "National Emergency": "bg-red-500/10 text-red-400",
}

const EmergencyServicesGrid = ({ categories = [], selectedCategory, setSelectedCategory }) => {
  return (
            ...
            >
              <div
                className={`mb-4 flex h-12 w-12 items-center justify-center rounded-xl ${
                  colorMap[category] || "bg-indigo-500/10 text-indigo-400"
                }`}
              >
                <Icon size={24} />
              </div>
              ...
              <p className="mt-2 text-sm text-slate-400">Verified emergency service contact.</p>

              <button
                onClick={() => setSelectedCategory(active ? "" : category)}
                className={`mt-5 w-full rounded-xl py-2.5 font-medium transition ${
                  active ? "bg-red-600 hover:bg-red-700" : "bg-indigo-600 hover:bg-indigo-700"
                }`}
              >
                {active ? "Clear Filter" : "View Details"}
              </button>
            </div>
          )
        })}
